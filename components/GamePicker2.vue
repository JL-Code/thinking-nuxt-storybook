<template>
  <!-- 游戏选择器组件S -->
  <ElPopover trigger="click" width="1000" placement="bottom-start"
    popper-class="game-picker__popover" :offset="2" :show-arrow="false" :persistent="false">
    <template #reference>
        ...
        <template v-if="steps.length">
          <div v-for="item in steps" class="w-full">
            <el-input :key="item.type" :placeholder="`请选择${item.typeName}`" @click="handleInputClick(item.type)"
              class="game-picker__input" readonly :model-value="item.label" :suffix-icon="Search" />
          </div>
        </template>
...
watch(
  ...
  async (val: KV<number>[], old?: KV<number>[]) => {
    console.warn("[game picker] watch model newValue", JSON.stringify(val));
    console.warn("[game picker] watch model oldValue", JSON.stringify(old));
...
function setDefaultValueWithQuery() {
  ...
  console.warn("[game picker] setDefaultValueWithQuery obj", obj);
...
async function setDefaultValue(params?: KV<number>[]) {
  console.warn("[game picker] setDefaultValue params", JSON.stringify(params), params);
  ...
  for (const param of params) {
    ...
    if (!node) {
      ...
      switch (param.key) {
        case "game":
          _gameId = param.value;
          console.warn(
            `[game-picker] 节点类型[${param.key}]的数据不存在，开始重新加载数据`
          );
          await loadData();
          break;
        default:
          ...
          console.warn("[game-picker] 服务器加载完毕", _gameId, Date.now());
          break;
      }
    }
    ...
    if (_node) {
      console.warn(`[game-picker] 设置默认值 type: ${param.key}`, param.value);
